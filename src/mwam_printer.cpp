...
void Printer::printScoreData(const ScoreData *aScoreData) {
	//DEBUG("Print out score data now!");
	// Is a high score.

	/*DEBUG("Hash: %s\n%d. %s - %d\nScans: %d\nDisrupted: %d\nKills: %d",
	      aScoreData->hashId, aScoreData->standing, aScoreData->initials,
	      aScoreData->finalScore, aScoreData->scans, aScoreData->disrupted, aScoreData->kills);

	print(" - ");
	println(aScoreData->finalScore);
	justify('C');
	println(aScoreData->hashId);
	justify('L');*/

	//println("/------------------------------\\|      A.F.T.E.R.G.L.O.W.      ||                              ||                -. .          ||           _____   ',' ,      ||         ,'     ,'   ', ',    ||       ,'     ,'      |  |    ||      \\       \\       |  |    ||        \\ /^\\   \\    ,' ,'    ||              \\   \\ ,' ,'     ||        / ~-.___\\.-'  ,'      ||      /   .______.- ~ \\       ||    /   /'          \\   \\     ||    \\./               \\/'     ||                              |");
	//println("|                              ||       by Jerry Belich        ||          @j3rrytron          ||                              ||        jerrytron.com         |\\------------------------------/");

	justify('C');
	println("A.F.T.E.R.G.L.O.W.");
	println("by Jerry Belich (@j3rrytron)");
	println("jerrytron.com\n");

	if (aScoreData->standing > 0) {
		print(aScoreData->standing);
		print(". ");
		println(aScoreData->initials);
		println("See your high score tweet at");
		println("@afterglowscores");
	}
	print("You killed ");
	println(aScoreData->finalScore);
	justify('L');
	println(kPrinterTearGently);
	feed(2);

	/*if (aScoreData->standing > 0) {
		print("|           ");
		if (aScoreData->standing != 10) {
			print(" ");
		}
		print(aScoreData->standing);
		print(". ");
		print(aScoreData->initials);
		println("            |");
	}
	//char score[6] = {0};
	//itoa(aScoreData->initials, score, 5);
	justify('C');
	print("You killed ");
	print(aScoreData->finalScore);
	justify('L');
	print("|        ");
	println("|                              ||       by Jerry Belich        ||          @j3rrytron          ||                              ||        jerrytron.com         |\\------------------------------/");
	//println(aScoreData->initials);
	...
	feed(2);*/
	/*DEBUG("Misses: %d\nShots: %d\nBombs: %d\nLife: %d\nBLight: %d",
	      aScoreData->misses, aScoreData->shotsFired, aScoreData->bombsUsed,
	      aScoreData->lifeRemaining, aScoreData->bLightKills);
	DEBUG("BMed: %d\n:BHeavy: %d\nBDis: %d\nBMis: %d\n:BBoss: %d",
	      aScoreData->bMediumKills, aScoreData->bHeavyKills, aScoreData->bDisruptorKills,
	      aScoreData->bMissileKills, aScoreData->bBossKills);
	DEBUG("FirstHit: %d\nFirstHitWave: %d\nSecondHit: %d\nSecondHitWave: %d\nKilledBy: %d\nWaveReached: %d",
	      aScoreData->firstHitBy, aScoreData->firstHitWave, aScoreData->secondHitBy, aScoreData->secondHitWave,
	      aScoreData->killedBy, aScoreData->waveReached);
	DEBUG("Start: %lu, End: %lu", aScoreData->startTime, aScoreData->endTime);*/
	/*Serial.print("Start: ");
	Serial.println(Time.timeStr(aScoreData->startTime));
	Serial.print("End: ");
	Serial.println(Time.timeStr(aScoreData->endTime));*/
}
